...
export function ChatMessage({ message, evolutionStage, currentPersona }: ChatMessageProps) {
  ...
  const formatPersonaState = (persona: ChatbotPersona | undefined): string => {
    if (!persona) return '';
    let str = `(Style: ${persona.responseStyle}, UI: ${persona.uiVariant}, Tone: ${persona.emotionalTone}, Know: ${persona.knowledgeLevel}, V: ${persona.affectiveState.valence.toFixed(1)}, A: ${persona.affectiveState.arousal.toFixed(1)}, Res: "${persona.resonancePromptFragment.substring(0,20)}..."`;
